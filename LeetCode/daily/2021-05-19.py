    ...
    def minMoves2(self, nums: List[int]) -> int:
        ...
        for idx, num in enumerate(nums):
            for i in range(len(arr)):
                if idx != i:
                    arr[i] += abs(nums[i] - num)
        return min(arr)


# TLE 27: 19604.430198669434
class Solution2:
    def minMoves2(self, nums: List[int]) -> int:
        arr = [0] * len(nums)
        for idx, num in enumerate(nums):
            arr[idx] = sum(map(lambda x: abs(num - x), nums))
        return min(arr)
...
class Solution:
    def minMoves2(self, nums: List[int]) -> int:
        ...
        min_val = float('inf')
        for idx, num in enumerate(nums):
            min_val = min(min_val, (total - num) - num * (h-1) + (num * idx - cum) * 2)
            cum += num
        return min_val
